...
/*   Updated: 2018/10/25 18:47:31 by mdambrev         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char		*dup_occu(char *str, int offset, int n_occu, char delim)
{
	int		end;
...
char		*dup_occu_by_delim(char *str, char delim, int n_occu)
{
	...
	while(str[x] != 0)	
	{
		if(str[x] == delim)
		{
		 ...
		 if(cmp == n_occu)
			 return(dup_occu(str, x, n_occu, delim));
		x++;
	}
	return(NULL);
}

char		*get_occu(char *str, int offset, int n_occu, char delim)
{
	while(str[offset] == delim)
		offset++;
	return(str + offset);
...
char		*get_occu_by_delim(char *str, char delim, int n_occu)
{
	...
	while(str[x] != 0)	
	{
		if(str[x] == delim)
		{
		 ...
		 if(cmp == n_occu)
			 return(get_occu(str, x, n_occu, delim));
		x++;
	}
	return(NULL);
}
