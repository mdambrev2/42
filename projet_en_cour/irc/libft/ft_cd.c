...
/*   Updated: 2018/11/23 06:52:01 by mdambrev         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int					set_old_pwd(char *old_pwd, char *pwd,
								const char *path, char *racine_serv)
{
	if (old_pwd[0] == '0')
		return (-2);
	if (path[0] == '-' && path[1] != '\0')
...
int					ft_cd(const char *path, int sec, char *racine_serv)
{
	...
	else if ((error = set_old_pwd(old_pwd, pwd, path, racine_serv)) < 0)
		return (put_error(error, &pwd, &absolute_path));
	ft_strdel(&pwd);
